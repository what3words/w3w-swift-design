...
//  }
  ...
  public func addSlashesIfAddress(color: W3WColor = .red) -> W3WString {
    if W3WRegex.isPossible3wa(text: asString()) {
      string = removeLeadingSlashes().string
      return withSlashes(color: color)
    }
    ...
    string.enumerateAttributes(in: NSRange(location: 0, length: string.length)) { (attributes, range, stop) in
      attributes.forEach { (key, value) in
        if let color = value as? UIColor {
          let c = W3WCoreColor(uiColor: color)
          pieces.append("[" + string.mutableString.substring(with: range) + "]" + "(\(c))")
        } else {
          pieces.append(String(describing: key) + ": " + String(describing: value))
        }
      }
    }
