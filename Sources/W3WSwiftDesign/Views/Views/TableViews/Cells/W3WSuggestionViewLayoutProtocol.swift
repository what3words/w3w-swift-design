...
extension W3WSuggestionViewLayoutProtocol {
  ...
  func instantiateUIElements() {
    ...
    wordsLabel = W3WLabel(fontStyle: .body, theme: theme)
    add(view: wordsLabel!, position: wordsPosition)

    nearestPlaceLabel = W3WLabel(fontStyle: .caption1, theme: theme)
    add(view: nearestPlaceLabel!, position: .place(below: wordsLabel!, margin: spacing(), height: descriptionLabelHeight()))

    distanceLabel = W3WLabel(fontStyle: .caption1, theme: theme)
  ...
  func update(with: W3WSetTypes = .cells) {
    ...
    if let colors = theme?[with]?.colors ?? W3WTheme.standard[.base]?.colors {
  ...
  func wordsPosition(parent: UIView?, this: W3WViewProtocol) -> CGRect {
    let padding = theme?[.cells]?.styles?.padding?.value ?? W3WPadding.medium.value
...
//        self.wordsLabel?.attributedText = W3WString(suggestion.words ?? "").withSlashes(color: theme?.colors?.findTint() ?? .slash).asAttributedString()
