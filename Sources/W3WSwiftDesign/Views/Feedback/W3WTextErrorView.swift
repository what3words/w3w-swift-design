...
public class W3WTextErrorView: W3WMessageView {
  ...
  /// initialize the UI
  override func setupUI(colors: W3WColorScheme?) {
  ...
  override func updateColours() {
    if let c = W3WBaseColors.error.background {
    ...
    label?.attributedText = W3WString(label?.text ?? "").style(color: W3WBaseColors.error.foreground, font: font).asAttributedString()
  }
