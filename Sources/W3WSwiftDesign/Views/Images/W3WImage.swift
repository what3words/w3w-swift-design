...
public class W3WImage {
  ...
  public init(systemName: String, colors: W3WColorSet) {
    self.imageSource = .system(systemName)
    self.colors = colors
  }
  
  
  public init(drawing: W3WDrawing, colors: W3WColorSet) {
    imageSource = .drawing(drawing)
    self.colors = colors
  }
  
  
  public init(file: String, colors: W3WColorSet) {
  ...
  func from(drawing: W3WDrawing, size: CGSize) -> UIImage {
    return UIImage(cgImage: drawing.asCGImage(size: size, colors: colors)!)
  }
  ...
  func from(file: String, size: CGSize) -> UIImage {
    if let maskImage = UIImage(named: file, in: Bundle.module, compatibleWith: nil)?.withRenderingMode(.alwaysTemplate) {
      ...
      if let color = colors.tint?.current.cgColor {
